(() => {
  ...
  // extensions/currency-conversion-banner/src/Checkout.jsx
...
${errorInfo.componentStack}`);
  ...
  function App() {
    ...
    const { cost, note } = useApi();
    const totalAmount = useSubscription(cost.totalAmount);
    const [ves, setVes] = (0, import_react11.useState)(null);
    (0, import_react11.useEffect)(() => {
      const fetchVes = () => __async(this, null, function* () {
        const response = yield fetch(`https://api.coinbase.com/v2/exchange-rates`);
        const allRates = yield response.json();
        ...
        setVes(moneyConversion.replace(/\B(?=(\d{3})+(?!\d))/g, ","));
        console.log(note);
      });
      fetchVes();
    });
    ...
    const fullDescription = `${description === void 0 ? "" + ves : description.replace("[[amount]]", ves)}`;
    if (ves) {
      return /* @__PURE__ */ (0, import_jsx_runtime4.jsx)(Banner2, { title, status, collapsible, children: fullDescription });
    } else {
      return null;
    }
  }
})();
//# sourceMappingURL=currency-conversion-banner.js.map
